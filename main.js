...
global.channelLink = "https://whatsapp.com/channel/0029VbAbjnJDuMRjBOCxfJ0j";
...
const channelInfo = {
    contextInfo: {
        ...
        forwardedNewsletterMessageInfo: {
            newsletterJid: '120363419026503717@newsletter',
            newsletterName: 'CRIMINAL BOT',
            serverMessageId: -1
        }
    }
};
        ...
        if (isBanned(senderId) && !userMessage.startsWith('.unban')) {
            ...
            if (Math.random() < 0.1) {
                await sock.sendMessage(chatId, {
                    text: '🛑 CRIMINAL BOT: You’ve been officially blocked from commands. Talk to an admin.',
                    ...channelInfo
                });
            }
            return;
        ...
        if (isGroup && isAdminCommand) {
            ...
            if (
                ...
                if (!isSenderAdmin && !message.key.fromMe) {
                    await sock.sendMessage(chatId, {
                        text: '😈 CRIMINAL BOT: Nice try, peasant. This button belongs to the kings and queens (admins).',
                        ...channelInfo
                    });
                    return;
                }
        ...
        switch (true) {
            case userMessage === '.simage': {
                ...
                if (quotedMessage?.stickerMessage) {
                    ...
                    await sock.sendMessage(chatId, { text: '🎯 Tip from CRIMINAL BOT: Just reply to any sticker with .simage and watch the magic happen ✨.', ...channelInfo });
                }
                break;
            }
            ...
            case userMessage.startsWith('.mute'):
                ...
                if (isNaN(muteDuration)) {
                    await sock.sendMessage(chatId, { text: '🕒 CRIMINAL BOT Tip: Minutes must be valid! Try: .mute 10\neg to mute 10 minutes\n.mute 10', ...channelInfo });
                } else {
...
case (userMessage && userMessage.trim().toLowerCase() === '.whoisgay'):
  ...
  break;
            ...
            case userMessage.startsWith('.mode'):
                ...
                try {
                    ...
                    await sock.sendMessage(chatId, { text: `🔄 Mode updated — CRIMINAL BOT is now in *${action}* mode.`, ...channelInfo });
                } catch (error) {
  ...
  break;
            ...
            case userMessage.startsWith('.antilink'):
                if (!isGroup) {
                    await sock.sendMessage(chatId, {
                        text: '🏠 CRIMINAL BOT: This command works only inside groups.',
                        ...channelInfo
                    });
                    return;
                }
                if (!isBotAdmin) {
                    await sock.sendMessage(chatId, {
                        text: '⚠️ CRIMINAL BOT: Make me an admin first so I can do my magic. 🪄',
                        ...channelInfo
                    });
                    return;
                }
    ...
    break;
            ...
            case userMessage.startsWith('.welcome'):
                if (isGroup) {
                    ...
                    await sock.sendMessage(chatId, { text: '🏠 CRIMINAL BOT: This command can only be used inside groups...', ...channelInfo });
                }
                break;
            ...
            case userMessage.startsWith('.antibadword'):
                ...
                if (!isBotAdmin) {
                    await sock.sendMessage(chatId, { text: '*👑 Give CRIMINAL BOT admin powers before trying this!*', ...channelInfo });
                    return;
                }
            ...
            case userMessage.startsWith('.chatbot'):
                if (!isGroup) {
                    await sock.sendMessage(chatId, { text: '👥 CRIMINAL BOT says: You gotta be in a group to run this..', ...channelInfo });
                    return;
                }
                ...
                if (!chatbotAdminStatus.isSenderAdmin && !message.key.fromMe) {
                    await sock.sendMessage(chatId, { text: '*🚫 CRIMINAL BOT: Only group admins or the bot owner can use this command. 👑*', ...channelInfo });
                    return;
                }
...
// Update commands for update bot auto 
     ...
     //           break;
            ...
            case userMessage === '.groupinfo' || userMessage === '.infogp' || userMessage === '.infogrupo':
                if (!isGroup) {
                    await sock.sendMessage(chatId, { text: '👥 CRIMINAL BOT says: You must be in a group to use this.', ...channelInfo });
                    return;
                }
...
case userMessage.startsWith('.sudo'):
                ...
                async function groupJidCommand(sock, chatId, message) {
                    ...
                    if (!groupJid.endsWith('@g.us')) {
                        return await sock.sendMessage(chatId, {
                            text: "👥 CRIMINAL BOT says: You must be in a group to use this."
                        });
                    }
        ...
        if (chatId) {
            await sock.sendMessage(chatId, {
                text: '💥 Error! CRIMINAL BOT couldn’t complete your request.',
                ...channelInfo
            });
        }
    }
...
async function handleGroupParticipantUpdate(sock, update) {
    try {
        ...
        if (action === 'add') {
            ...
            const welcomeMessage = welcomeData?.message || '👑 CRIMINAL BOT: Hail {user}! The group just got cooler with you here!';
            const channelId = welcomeData?.channelId || '120363419026503717@newsletter';
        ...
        if (action === 'remove') {
            ...
            const channelId = goodbyeData?.channelId || '120363419026503717@newsletter';
